        ...
        : IHostedService
{
    ...
    private void OnStarted()
    {
        try
        {
            ...
            {
                case "expr":
                    var parsed = Parser.Expr.ExpressionParser.ParseExpression(input);
                    logger.LogInformation("parsed: {}", parsed);
                    logger.LogInformation("value: {}", parsed.Compile()());
                    break;

                case "xml":
                    var parsedXml = Parser.Xml.XmlParser.Document.Parse(input);
                    logger.LogInformation("parsed: {}", parsedXml);
                    break;

                case "csv":
                    var parsedCsv = Parser.Csv.CsvParser.Csv.Parse(input);
                ...
                case "mydsl":
                    var parsedMyDSL = Parser.MyDSL.MyDSLParser.Questionnaire.Parse(input);
                    logger.LogInformation("parsed: {}", parsedMyDSL);
                    break;

                case "bat":
                    var parsedWindowsBat = Parser.WindowsBatch.WindowsBatchParser.BatchFile.Parse(input);
